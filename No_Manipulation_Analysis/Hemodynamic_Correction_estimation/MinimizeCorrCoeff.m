function [correctionConstant]= MinimizeCorrCoeff(GCaMP,CBV,Fs)
%Inputs:
...
figure(101);
ax1=subplot(2,1,1);plot(plotTime,CBV,'Color','m','LineWidth',1);
hold on; plot(plotTime,GCaMP,'Color','g','LineWidth',1); xlabel('Time (sec)'); ylabel('a.u'); title('Uncorrected GCaMP and TRITC');
 legend({'TRITC','GCaMP'});
[initCorrCoeffs]=xcorr(detrend(GCaMP),detrend(CBV),0,'coeff');
...
adjustedGCaMP=GCaMP+correctionConstant*CBV;

figure(101);
ax2=subplot(2,1,2);plot(plotTime,CBV,'Color','m','LineWidth',1);
hold on; plot(plotTime,adjustedGCaMP,'Color','g','LineWidth',1); 
xlabel('Time (sec)'); ylabel('a.u'); title('Corrected GCaMP and TRITC');
legend({'TRITC','GCaMP'});
linkaxes([ax1,ax2],'x'); xlim([0 7000]);

[adjCorrCoeffs]=xcorr(detrend(adjustedGCaMP),detrend(CBV),0,'coeff');
end
...
correctedGCaMP=GCaMP+(theScale*CBV); % attempt to remove reflected copy of TRITC signal
